...
const Sidebar = ({ users, currentUser, onSelectUser, onLogout, activeThreadUserId, availableRoles, changelog }) => {
  ...
  const usersToDisplayInThreads = canViewOtherThreads
    ? users.filter(u => {
  ...
  const CollapsibleSection = ({ title, isVisible, onToggle, children }) => (
    <div className="py-2">
      <button onClick={onToggle} className="flex items-center justify-between w-full px-2 text-left">
      ...
      {isVisible && <div className="mt-2 space-y-1">{children}</div>}
    </div>
  );
  ...
  return (
    <div className="flex flex-col w-64 h-screen bg-gray-800 text-white">
      ...
      <div className="flex-1 overflow-y-auto px-2">
        ...
        <CollapsibleSection title="Changelog" isVisible={isChangelogVisible} onToggle={() => setIsChangelogVisible(!isChangelogVisible)}>
          <div className="p-2 space-y-3 text-xs text-gray-300 bg-gray-900/50 rounded-md">
            {changelog.map(entry => (
              <div key={entry.version}>
                <p className="font-bold text-white">{entry.version} <span className="font-normal text-gray-400">({entry.date})</span></p>
                <ul className="pl-3 mt-1 list-disc list-inside">
        ...
        <CollapsibleSection title={canViewOtherThreads ? 'Wątki członków' : 'Mój wątek'} isVisible={isThreadsVisible} onToggle={() => setIsThreadsVisible(!isThreadsVisible)}>
        ...
        <CollapsibleSection title="Lista Członków" isVisible={isMembersVisible} onToggle={() => setIsMembersVisible(!isMembersVisible)}>
          {users.map(user => (
             <div key={user.id} className="px-3 py-2 text-sm text-gray-300 rounded-md bg-gray-900/30">
                <p className="font-semibold text-white">{user.nickname}</p>
                <div className="flex justify-between text-xs text-gray-400">
                    <span>{user.role}</span>
                    <span>ID: {user.staticid}</span>
                </div>
            </div>
          ))}
        </CollapsibleSection>
