...
const initialConfig = {
  ...
  availableRoles: [
      { name: 'Lider', canViewThreads: true, isThreadVisible: false, canApprove: true, canReject: true, canPost: false },
      { name: 'V-lider', canViewThreads: true, isThreadVisible: true, canApprove: true, canReject: true, canPost: true },
      { name: 'Member', canViewThreads: false, isThreadVisible: true, canApprove: false, canReject: false, canPost: true }
  ],
...
const initializeDatabase = async () => {
  ...
  try {
    ...
    // Sprawdź, czy tabela 'available_roles' ma nową kolumnę
    const checkColumn = await client.query("SELECT column_name FROM information_schema.columns WHERE table_name='available_roles' AND column_name='canpost'");
    
    if (checkColumn.rowCount > 0) {
        console.log('Database schema is up to date.');
        // Opcjonalnie: można dodać logikę synchronizacji, jeśli to konieczne
        return;
    }

    console.log('Database schema is outdated. Re-initializing...');
    ...
    const schema = `
      ...
      CREATE TABLE available_roles (name TEXT PRIMARY KEY, canViewThreads BOOLEAN, isThreadVisible BOOLEAN, canApprove BOOLEAN, canReject BOOLEAN, canPost BOOLEAN);
    `;
    ...
    for (const role of initialConfig.availableRoles) {
        await client.query('INSERT INTO available_roles (name, canViewThreads, isThreadVisible, canApprove, canReject, canPost) VALUES ($1, $2, $3, $4, $5, $6)',
        [role.name, role.canViewThreads, role.isThreadVisible, role.canApprove, role.canReject, role.canPost]);
    }
...
app.post('/api/contracts', authenticateToken, async (req, res) => {
    if (!req.user.permissions.canpost) return res.status(403).send('Brak uprawnień do dodawania kontraktów.');
